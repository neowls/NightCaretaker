...
- 시점은 끝까지 Full First Person View를 유지한다.
...
- 레이캐스트 기반 포커스
- 문, 스위치, 배전반, 메모, 민원 보드, 공구함에 공통 규칙 적용
- 상호작용 타입은 `Use`, `Inspect`, `Read`, `Report` 정도로 제한
- 상호작용 카메라와 손전등/도구 사용은 Full First Person View 기준으로 정렬한다
...
- `ANCCharacterBase`: 이동, 시점, 상호작용 추적, 손전등
...
### 8.4 Full First Person View 방침

- 플레이어 체험은 카메라 분리형이 아니라 전신 기반 1인칭을 기준으로 한다.
- 하나의 풀바디 캐릭터 메쉬를 기준으로 카메라, 그림자, 도구 위치를 정렬한다.
- 별도의 1인칭 전용 팔 리그는 초기 범위에서 만들지 않는다.
- 손전등, 문 열기, 스위치 조작은 모두 1인칭 시야와 캐릭터 애니메이션이 어긋나지 않도록 맞춘다.
- 플레이어가 아래를 봤을 때 자신의 몸과 도구가 자연스럽게 보이는 구성을 목표로 한다.
...
2. Full First Person View 기준 캐릭터, 카메라, 손전등 정렬
