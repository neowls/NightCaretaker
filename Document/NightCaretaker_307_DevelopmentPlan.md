...
- v1 물리 상호작용은 `Physics Handle` 기반 홀드/드롭으로 제한하고, 과한 원거리 흡입은 제외한다.
- 대상은 전용 마커 컴포넌트가 붙은 물리 프랍으로 한정해 디자이너가 명시적으로 제어한다.
- 기본 입력은 우클릭 홀드이며, 입력을 떼면 즉시 프랍을 놓는다.
...
- 문은 일반 프랍과 분리된 전용 물리 문 액터로 구현하고, `LockedClosed`, `LatchedClosed`, `FreeSwing`, `GrabDragging` 네 상태만 사용한다.
...
- 문 direct grab 은 플레이어의 공유 `Physics Handle`을 사용하고, 문짝을 카메라 앞으로 옮기지 않고 힌지를 따라 회전시키는 방식으로 제한한다.
